...
Parse Garmin `.fit`, `.tcx`, and `.gpx` activity files into a PostgreSQL database. Designed for personal fitness data warehousing — extract lap, record, session, and event data from your watch exports and store them in a relational schema for analysis.
...
- Parse `.tcx` and `.gpx` files (e.g. Strava exports)
...
Run `schema_garmin_data.sql` to create the tables.
...
```env
DB_UI_LOCAL=postgresql://user:password@localhost:5432/your_database
SCHEMA=your_schema
...
### Watch files (`.fit` only)

For files exported directly from a Garmin watch via USB:
...
- A database connection is required (activity IDs are generated by the database)
...
| `backfill_lap_power.py`       | One-time script to populate power columns on existing laps            |
